...
    // If new audio was picked, update it; otherwise keep existing or null
    if (audioFile) {
      payload.audioBlob = audioFile;
    } else if (isEditing && initialData.audioBlob) {
...
            type="button"
            onClick={onClose} 
            className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-full transition-colors active:scale-95"
            title={t.back}
          >
            <ArrowLeft size={22} />
          </button>
...
                  {audioFile 
                    ? audioFile.name 
                    : (isEditing && initialData.audioBlob 
                        ? t.keepExistingAudio 
                        : t.chooseAudio)}
                </span>
...
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 border border-slate-300 dark:border-slate-700 rounded-xl font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
              {t.cancel}
            </button>
            <button
              type="submit"
              className="px-6 py-2.5 bg-blue-700 hover:bg-blue-800 text-white font-medium rounded-xl shadow-md transition-colors flex items-center gap-2 active:scale-95"
            >
